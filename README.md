...
### 이 저장소에 없지만 필요한 의존 파일 (핸드오프 시 꼭 챙길 것)
...
- ARM_SDK 명령을 먹이려면 로봇을 **제어 가능한 상태**로 올려야 함: 리모컨으로 세운 뒤(`L2+B`→`L2+UP`→`R2+A`), 필요 시 **디버그 모드(`L2+R2`)** 또는 내장 모션매니저 제어권 핸드오프. **정확한 순서는 현장 운영자에게 확인.**
...
**한계**
- **grasp 미전이:** 시뮬의 잡기는 운동학적 부착(가짜)이라 실물에 안 넘어감. 현재 실물 코드(ARM_SDK)는 손가락(Dex3)을 제어하지 않음 → 실물은 "팔 동작 시늉"만.
